...
4. **Настройте BASE_PATH в config.php:**
   - Если сайт в корне домена: `define('BASE_PATH', '');`
   - Если в подпапке (например `/gst`): `define('BASE_PATH', '/gst');`
5. **Создайте первого администратора** через `[BASE_PATH]/auth/register_admin.php`
6. **Вход** через `[BASE_PATH]/auth/login.php`
7. Дальнейшая работа — через меню и дашборд.
